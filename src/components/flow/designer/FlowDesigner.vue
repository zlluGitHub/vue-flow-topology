<template>
  <div style="height: 100%;">
    <a-layout class="container">
      <a-layout-sider :width="siderLeftWidth" theme="light" class="select-area">
        <a-row>
          <div class="logo">
            <h1>flow</h1>
          </div>
        ...
        <!--  <a-row>
         <a-checkable-tag v-model="tag.checked0" @change="toggleNodeShow0" class="tag">工具</a-checkable-tag>
          <div class="collapse" @click="toggleNodeShow0">
            <p>
              <a-icon type="setting" />
              <span>工具</span>
            </p>
            <a-icon v-if="tag.toolShow" type="caret-down" />
        ...
        <a-row>
          ...
          <div class="collapse" @click="toggleNodeShow1">
            <p>
              <a-icon type="setting" />
              <span>基础节点</span>
            </p>
            <a-icon v-if="tag.commonNodeShow" type="caret-down" />
            <a-icon v-else type="caret-left" />
          </div>
          <div align="center">
        ...
        <a-row>
          ...
          <div class="collapse" @click="toggleNodeShow2">
            <p>
              <a-icon type="setting" />
              <span>高级节点</span>
            </p>
            <a-icon v-if="tag.highNodeShow" type="caret-down" />
            <a-icon v-else type="caret-left" />
          </div>
          <div align="center">
        ...
        <a-row>
          ...
          <div class="collapse" @click="toggleNodeShow3">
            <p>
              <a-icon type="setting" />
              <span>泳道节点</span>
            </p>
            <a-icon v-if="tag.laneNodeShow" type="caret-down" />
            <a-icon v-else type="caret-left" />
          </div>
          <div align="center">
        ...
        <div class="bar-arrow-left" @click="handleShrink('left')">
          <a-icon :type="siderLeftWidth!==0?'double-left':'double-right'" />
        </div>
      </a-layout-sider>
      ...
      <a-layout>
        <div class="content-header">
          ...
          <div class="header-left">
            <!-- 节点菜单（顶部布局） -->
            <a-layout-header v-if="siderLeftWidth==0" class="header-option header-warp">
              <a-tooltip
                v-for="(commonNode, index) in field.commonNodes"
                :key="index+'tp'"
                :title="commonNode.nodeName"
                placement="bottom"
              >
                <div class="node-item item-active" :type="commonNode.type" belongto="commonNodes">
                  <a-icon :type="commonNode.icon" />
                </div>
              </a-tooltip>
            </a-layout-header>
            ...
            <a-layout-header class="header-option">
              <div v-for="(tool, index) in field.tools" :key="index+'tool'">
                <!-- 下拉菜单 -->
                <a-dropdown v-if="tool.type==='connection'" :trigger="['click']">
                  <a-button
                    :icon="tool.icon"
                    ...
                    :type="currentTool.type == tool.type ? 'primary': 'default'"
                    @click="selectTool(tool.type)"
                  >{{lineObj.name}}</a-button>
                  <a-menu slot="overlay">
                    <a-menu-item
                      v-for="(item, i) in tool.shape"
                      :key="i+'line'"
                      @click="handleSetFlowType(item)"
                    >
                      <span>{{item.name}}</span>
                    </a-menu-item>
                  </a-menu>
          ...
          <a-layout-header class="header-option">
            ...
            <a-tooltip title="测试" placement="bottom">
              <a-button @click="openTest" class="header-option-button" size="small" icon="code"></a-button>
            </a-tooltip>
            <!-- <a-popconfirm
              title="请选择帮助项："
              placement="bottom"
        ...
        <a-layout-footer class="foot">
          <span>flow {{ info.version }}, Powered by {{ info.author }}</span>
        </a-layout-footer>
      </a-layout>
      <a-layout-sider
        :width="siderRightWidth"
        theme="light"
        class="attr-area"
...
export default {
  ...
  mounted() {
    ...
    // 		message: '欢迎使用flow流程设计器!',
    // 		description: <p>
    // 						当前版本：1.0.5<br />
    // 						Powered by ZSFN<br />
    // 						码云：<a target="blank" href="https://github.com/zlluGitHub">flow流程设计器</a>
    // 					</p>,
  ...
  data() {
    return {
      ...
      info: {
        version: "1.0.0",
        author: "zll",
        gitee: "https://github.com/zlluGitHub",
      },
      tag: {
        checked0: true,
        checked1: true,
  ...
  methods: {
    toggleNodeShow0(flag) {
      this.tag.toolShow = !this.tag.toolShow;
      // if (!flag) {
    ...
    toggleNodeShow1(flag) {
      this.tag.commonNodeShow = !this.tag.commonNodeShow;
      // if (!flag) {
    ...
    toggleNodeShow2(flag) {
      this.tag.highNodeShow = !this.tag.highNodeShow;
      // if (!flag) {
    ...
    toggleNodeShow3(flag) {
      this.tag.laneNodeShow = !this.tag.laneNodeShow;
      // if (!flag) {
    ...
    handleSetFlowType(v) {
      this.lineObj = v;
      flowConfig.jsPlumbInsConfig.Connector[0] = v.type;
    },
    getBrowserType() {
    ...
    initNodeSelectArea() {
      ...
      $(document).ready(function () {
        $(".node-item").draggable({
          ...
          cursorAt: {
            top: 16,
            left: _this.siderLeftWidth === 0 ? 10 : 60,
          },
          containment: "window",
          revert: "invalid",
        });
      ...
      document.onkeydown = function (e) {
        ...
        switch (key) {
          ...
          case flowConfig.shortcut.connTool.code:
            ...
            break;
          case flowConfig.shortcut.deleteTool.code:
            that.$refs.flowArea.deleteNode();
            break;
