...
export default {
  ...
  methods: {
    handlerMousedown(e, item, i, n) {
      let event = e || window.event || event;
      let nodeEle = document.getElementById(`node-${i + "-" + n}`);
      ...
      let nodeTypeEle = document.getElementById(item.type + "-" + i + "-" + n);
      let nodeObj = nodeTypeEle.getBoundingClientRect();
      this.nodePosition = {
        offsetX: event.x - nodeObj.left,
        offsetY: event.y - nodeObj.top,
      };
      this.copyNode = nodeTypeEle.cloneNode(true);
      this.copyNode.className += " copy";
      this.copyNode.style.top = `${event.y - this.nodePosition.offsetY}px`;
      this.copyNode.style.left = `${event.x - this.nodePosition.offsetX}px`;
      nodeTypeEle.parentNode.appendChild(this.copyNode);
    },
  },
};
</script>
